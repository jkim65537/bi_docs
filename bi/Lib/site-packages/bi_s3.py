...
class S3BI(object):
    def __init__(self, bucket_name="pitchbook-snowflake"):
        """Easy way to read and write files from BI-related S3 buckets
        ...
        if self.bucket_name == "pitchbook-snowflake":
            ...
            raise ValueError(
                "bucket_name: {} not supported yet".format(connection))
    ...
    def read_from_s3(self, filepath):
        ...
        if ".gz" in filepath.lower():
            ...
            if ".csv" not in filepath.lower():
                df = pd.read_csv(output, header=None, error_bad_lines=False,
                    low_memory=True)
            else:
        ...
        if isinstance(col_names, list) and "column_names.csv" not in filepath:
    ...
    def _get_temp_path(self, file_type="gz"):
        ...
        if op_sys == "Windows":
            temp_path = "C:/Users/{username}/tempfile.{filetype}".format(
                username=username, filetype=file_type
            )
        elif op_sys == "Linux":
            temp_path = "/home/{username}/tempfile.{filetype}".format(
                username=username, filetype=file_type
            )
        else:
            raise OSError(
